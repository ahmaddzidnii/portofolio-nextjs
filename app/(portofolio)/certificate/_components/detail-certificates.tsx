...
const DetailCertificate = ({ title, img, link, credentials, summary }: DetailCertificateProps) => {
  return (
    <article className="w-full mb-8 flex items-center justify-between rounded-3xl border border-solid border-yellow-400 bg-white dark:bg-[#0a0a0a]  shadow-2xl p-12 relative rounded-br-2xl lg:flex-col lg:p-8 xs:rounded-2xl xs:rounded-br-2xl xs:p-4 ">
      ...
      <div className="w-1/2 flex flex-col items-start justify-between pl-6 lg:w-full lg:pl-0 lg:pt-6  ">
        ...
        <div className="mt-2">
          <Link href={link} target="_blank" className=" rounded-lg bg-slate-900 text-white px-6 p-2 text-lg font-semibold text-light sm:px-4 sm:text-base">
            Visit Certificate
          </Link>
        </div>
      </div>
